...
if "%~1"=="" (
  echo Usage: run-sonar-separate.bat ^<SONAR_HOST_URL^> ^<SONAR_TOKEN^> [PROJECT_KEY_PREFIX]
  exit /b 1
)
...
echo Separate SonarQube scans completed.
exit /b 0
...
sonar-scanner ^
  "-Dsonar.host.url=%SONAR_HOST_URL%" ^
  "-Dsonar.token=%SONAR_TOKEN%" ^
  "-Dsonar.projectBaseDir=sprint frontend\founderlink" ^
